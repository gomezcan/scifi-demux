...
def plan_chunks(raw_dir: Path, library: str, work_root: Path, chunks: int) -> Path:
    ...
    for i, (r1p, r2r1p, r3p, r2r3p) in enumerate(
        zip(r1_parts, r2r1_parts, r3_parts, r2r3_parts),
        start=1,
    ):
...
def worker_chunk(
    ...
    threads = (
        int(os.environ.get("SLURM_CPUS_PER_TASK") or os.environ.get("NSLOTS") or 1)
        if mode == "hpc"
        else 1
    )
    ...
    if not (sent_dir / (umi_ok.name + ".ok.json")).exists():
        r1_stats = umi_extract_pair(
            read_keep=Path(r1_raw),
            mate_in=Path(r2_for_r1),
            out_fastq_gz=r1_bc1,
            threads=threads,
        )
        r3_stats = umi_extract_pair(
            read_keep=Path(r3_raw),
            mate_in=Path(r2_for_r3),
            out_fastq_gz=r3_bc1,
            threads=threads,
        )
        ...
        # metrics
        _write_json(
            metrics_dir / f"chunk_{chunk_id:03d}.umi.json",
            {"r1": r1_stats, "r3": r3_stats},
        )

    # 2) Cutadapt rename/clip/trim
    if not (sent_dir / (cut_ok.name + ".ok.json")).exists():
        ca_stats = cutadapt_append_tn5_to_name(
            r1_in=r1_bc1,
            r3_in=r3_bc1,
            r1_out=r1_bc2,
            r3_out=r3_bc2,
    ...
    if not (sent_dir / (demux_ok.name + ".ok.json")).exists():
        ...
        demux_r1 = demux_by_split_bc(
            layout_file=layout_path,
            sample_well_map=design,
            input_fastq_gz=r1_bc2,
            out_dir=corr_dir,
        )
        demux_r3 = demux_by_split_bc(
            layout_file=layout_path,
            sample_well_map=design,
            input_fastq_gz=r3_bc2,
            out_dir=corr_dir,
        )
...
def run_step1_local(
    ...
    cmd = (
        f"seq 1 {chunks} | parallel -j {parallel_jobs} "
        f"scifi-demux step1 worker-chunk --plan {plan} --array-id {{}} --mode local"
    )
    if design:
        cmd += f" --design {design}"
    if layout:
    ...
    # Merge when done (also aggregates QC via _aggregate_counts_only)
...
def merge_library(library: str, work_root: Path) -> None:
    ...
    # Aggregate metrics into QC summaries (library + per-sample TSV/JSON)
...
def _write_progress(
    ...
    obj = {
        ...
        "counts": {
            ...
            "missing_indices": counts["missing"],
        },
        "state": state,
        "message": msg,
    }
    ...
    while True:
        ...
        if counts["total"] > 0 and counts["dem"] >= counts["total"]:
            ...
            _write_progress(
                ...
                max_wait_sec,
                "qc",
                "Merge complete; running MultiQC",
                started,
            )
            ...
            return
        msg = (
            f"{counts['dem']}/{counts['total']} chunks complete; "
            f"missing={','.join(map(str, counts['missing']))}"
            if counts["total"]
            else "waiting for plan"
        )
        _write_progress(
            work_root,
            library,
...
def run_step1_hpc(
    ...
    *,
    ...
    if follow:
        ...
        # merge_library() (called inside wait_and_maybe_merge) already runs
        # _aggregate_counts_only, so no extra call is needed here.
    return plan
